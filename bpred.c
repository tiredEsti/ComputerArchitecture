             ...
             unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPredCascade:
    /* bimodal component */
    pred->dirpred.bimod =
        bpred_dir_create(BPred2bit, bimod_size, 0, 0, 0, 1);

    /* 2-level component */
    pred->dirpred.twolev =
        bpred_dir_create(BPred2Level, 1, l2size, shift_width, 1, 1); /*l1size y xor  se pone siempre a 1 para gshare + uso_gshare se inicializa a 1*/
allocate ret
    break;
...
bpred_dir_create(
    ...
    unsigned int xor,         /* history xor address flag */
    /*NUEVO*/
    char uso_cascade) /*indica si estamos usando cascade o no, solo sera 1 para cascade*/
{
  ...
  case BPred2Level:
  {
    ...
    if (uso_cascade == 1)
    {
      for (cnt = 0; cnt < l2size; cnt++)
      {
        ...
        pred_dir->config.two.shiftregs[cnt] = 3;
      }
    }
    break;
  }
                                                     ...
                                                     * used on mispredict recovery */
{
  ...
  case BPredCascade:
    if ((MD_OP_FLAGS(op) & (F_CTRL | F_UNCOND)) != (F_CTRL | F_UNCOND))
    {
      ...
      if (twolev == 4)
      {                               // Això vol dir que no hi ha entrada
        dir_update_ptr->dir.meta = 0; // per a 2lvl, meta = 1, per a bimod, meta = 0;
        dir_update_ptr->pdir1 = bimod;
        dir_update_ptr->pdir2 = twolev;
      }
      else
      { // si si que hi ha entrada al 2lvl, la fem servir

        dir_update_ptr->dir.meta = 1; // per a 2lvl, meta = 1, per a bimod, meta = 0;
        dir_update_ptr->pdir1 = twolev;
        dir_update_ptr->pdir2 = bimod;
      }
    }
                  ...
                  struct bpred_update_t *dir_update_ptr) /* pred state pointer */
{
  ...
  if (MD_OP_FLAGS(op) & (F_CTRL | F_UNCOND))
    != (F_CTRL | F_UNCOND) &&
        (pred->class == BPred2Level || pred->class == BPredComb || (pred->class == BPredCascade && dir_update_ptr->dir.meta)) // NUEVO
    {
  ...
  // NUEVO: siempre se actualiza pdir1
  ...
  // NUEVO: pdir2 solo se actuliza cuando pdir1 es 2lvl (cuando es bimod, vaya)
